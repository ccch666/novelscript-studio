...
- DeepSeek 剧本生成：`POST /api/scripts/generate`。
- YAML Schema 校验：`POST /api/scripts/validate`。
- YAML 自动修复：`POST /api/scripts/repair`。
- 示例小说与示例输出：`GET /api/samples/novel`、`GET /api/samples/output`。
- 前端小说粘贴、`.txt` 上传、示例加载和章节分析。
- 前端展示生成结果的 Schema 校验状态和错误列表。
- 前端展示角色表和场景卡片。
- 前端支持 YAML 在线编辑、重新校验和导出。
- 前端支持加载示例 YAML 和展示改编报告。
- 前端支持工作流状态条、加载状态提示和移动端布局打磨。
- YAML Schema 文件与设计文档。
